...
import pandas as pd, uvicorn, bcrypt
...
CLIENT_NORMALIZE = {
    'KOCAR/ PICANHA GRILL':'KOCAR / PICANHA GRILL','KOCAR BRASIL':'KOCAR',
    'AÃ‡OUQUE.COM':'AÃ‡OUQUE.COM','PAPAGAIO':'PAPAGAIO','Papagaio':'PAPAGAIO',
    'GAÃšCHA':'GAÃšCHA','MERCATIL EXTRA':'MERCATIL EXTRA','RIO BRANCO':'RIO BRANCO',
    'BOLO DA SOGRA':'BOLO DA SOGRA','CASA DE CARNE':'CASA DE CARNE',
    'CASA DE PAES':'CASA DE PAES','DETROIT':'DETROIT',
    'AVULSOS':'AVULSOS (SEM CLIENTE)','SIMONE':'SIMONE',
...
DRIVER_NORMALIZE = {
    ...
    'JHON':'Jonatan','LUCAS':'Lucas','BRUNO':'Bruno','KLEYBERTH':'Kleyberth',
    'KLEY':'Kleyberth','LUIS':'Luis','LUÃS':'Luis','WILLIAM':'William','MONTEIRO':'Monteiro',
    'CRISLEY':'Crisley','KRYSLEY':'Crisley','CHRISLEY':'Crisley',
}
CLIENT_KEYWORDS = ['KOCAR','CASA','DETROIT','GAÃšCHA','GAUCHA','PICANHA',
                   'AÃ‡OUQUE','ACOUQUE','PAPAGAIO','MERCATIL','RIO BRANCO','BOLO','SIMONE','AVULSO']

def normalize_driver(raw):
    if not raw or str(raw).strip() in ['','nan','OK','PAGO']: return None
    raw_up=str(raw).strip().upper()
    m=re.search(r'[-â€“]\s*([A-ZÃÃ‰ÃÃ“ÃšÃ€Ã‚ÃŠÃ”ÃƒÃ•Ã‡]+)\s*$', raw_up)
    if m:
        name=m.group(1).title()
        return DRIVER_NORMALIZE.get(name.upper(), name)
    for key,canonical in DRIVER_NORMALIZE.items():
        if raw_up==key: return canonical
    return None

# â”€â”€ Parse avaria â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Importar Excel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
MONTH_NAMES={"JANEIRO":1,"FEVEREIRO":2,"MARÃ‡O":3,"MARCO":3,"ABRIL":4,"MAIO":5,
             "JUNHO":6,"JULHO":7,"AGOSTO":8,"SETEMBRO":9,"OUTUBRO":10,"NOVEMBRO":11,"DEZEMBRO":12}

def extract_nf(xl,sheet):
    recs=[]
    try:
        df=pd.read_excel(xl,sheet_name=sheet,header=None)
        hr=next((i for i,row in df.iterrows() if "DATA" in str(row.values).upper()),None)
        if hr is None: return []
        date=client=prod=nf=None
        for i in range(hr+1,len(df)):
            row=df.iloc[i]
            if pd.notna(row.iloc[1]) and hasattr(row.iloc[1],"year"): date=row.iloc[1]
            if pd.notna(row.iloc[2]) and str(row.iloc[2]).strip() not in ["","nan"]: client=str(row.iloc[2]).strip()
            if pd.notna(row.iloc[3]) and str(row.iloc[3]).strip() not in ["","nan","PRODUTO","PRODUTOS"]: prod=str(row.iloc[3]).strip()
            if pd.notna(row.iloc[4]):
                try: nf=str(int(float(row.iloc[4])))
                except (ValueError, TypeError):
                    nf_raw=str(row.iloc[4]).strip()
                    nf=nf_raw if nf_raw.lower() not in ["","nan"] else nf
            qt=row.iloc[5]; pu=row.iloc[6]; tot=row.iloc[7]
            if date and prod and pd.notna(qt) and pd.notna(tot):
                try:
                    t=float(tot); q=float(qt); p=float(pu) if pd.notna(pu) else 0
                    driver=normalize_driver(str(row.iloc[8])) if len(row)>8 and pd.notna(row.iloc[8]) else None
                    if t!=0 and prod.upper() not in ["NAN","TOTAL DO MÃŠS","TOTAL DO MES"]:
                        recs.append(dict(sale_type="NF",sale_date=date.strftime("%Y-%m-%d"),
                            client=client,product=norm_p(prod,"NF"),nf_number=nf,
                            quantity=q,unit_price=p,total=t,delivery_person=driver,source="excel"))
                except (ValueError, TypeError, AttributeError):
                    # linha mal-formada (texto onde esperava nÃºmero) â€” pula
                    pass
    except Exception as e:
        # erro mais grave (sheet vazia, encoding, etc.) â€” sinaliza no log
        print(f"âš ï¸  extract_nf falhou no sheet '{sheet}': {type(e).__name__}: {e}")
    return recs

def parse_retorno(text):
    """
    Interpreta a coluna 'RETORNO NF E ENTREGADOR' das planilhas PR.
    PadrÃµes aceitos: 'OK - LUCAS', 'OK - LUCAS E LUIS', 'NOTA NO LOCAL',
    'NOTA NO LOCAL - JUNIOR', 'JHON', 'OK', 'OK -LUCAS' (typos).
    Retorna (delivery_person, status_note) onde:
      - delivery_person: nome(s) normalizado(s), ou None
      - status_note: 'NOTA NO LOCAL', 'NOTA ENVIADA PELO WHATSAPP', etc. ou None
    """
    if not text: return (None, None)
    s = str(text).strip()
    if not s or s in ("-","RETORNO NF E ENTREGADOR"): return (None, None)
    # Ignora valores puramente numÃ©ricos (lixo)
    try:
        float(s.replace(",","."))
        return (None, None)
    ...
    su = s.upper()
    # Detecta status no inÃ­cio (NOTA ..., OK)
    status = None
    drivers_part = su
    for prefix in ("NOTA ENVIADA PELO WHATSAPP","NOTA NO LOCAL","NOTA"):
        if su.startswith(prefix):
            status = prefix
            rest = s[len(prefix):].lstrip()
            # Pode ter "- DRIVER" depois
            if rest.startswith("-") or rest.startswith("â€“"):
                drivers_part = rest.lstrip("-â€“ ").upper()
            else:
                drivers_part = ""
            break
    else:
        if su.startswith("OK"):
            ...
            if rest.startswith("-") or rest.startswith("â€“"):
                ...
                drivers_part = rest.upper()

    drivers_part = drivers_part.strip()
    if not drivers_part:
        return (None, status if status and status != "OK" else None)

    # MÃºltiplos entregadores separados por "E" ou "&" ou "/"
    parts = re.split(r"\s+E\s+|\s*&\s*|\s*/\s*", drivers_part)
    normalized = []
    for p in parts:
        p = p.strip()
        if not p: continue
        # Tira lixo trailing
        p = re.sub(r"[^\w\sÃÃ‰ÃÃ“ÃšÃ‚ÃŠÃ”ÃƒÃ•Ã‡]+$", "", p, flags=re.IGNORECASE)
        if not p: continue
        nm = DRIVER_NORMALIZE.get(p, p.title())
        if nm not in normalized:
            normalized.append(nm)
    driver = " e ".join(normalized) if normalized else None
    # Status sÃ³ guarda se nÃ£o for OK (que Ã© o default)
    note = status if status and status != "OK" else None
    return (driver, note)

def extract_pr(xl,sheet):
    """Extrai Produtor Rural com forward-fill de data E cliente. Captura tambÃ©m
    entregador e status da coluna 'RETORNO NF E ENTREGADOR'."""
    recs=[]
    try:
        df=pd.read_excel(xl,sheet_name=sheet,header=None)
        hr=next((i for i,row in df.iterrows() if "DATA" in str(row.values).upper()),None)
        if hr is None: return []
        hrow=df.iloc[hr]
        # Detectar colunas pelo cabeÃ§alho
        total_col=7; client_col=2; prod_col=3; nf_col=4; retorno_col=None
        for ci,v in enumerate(hrow.values):
            if pd.notna(v):
                lab=str(v).upper().strip()
                if lab=="TOTAL": total_col=ci
                elif lab=="CLIENTE": client_col=ci
                elif "PRODUTO" in lab: prod_col=ci
                elif lab in ("NF-E","NFE","NF","NOTA"): nf_col=ci
                elif "RETORNO" in lab or "ENTREGADOR" in lab: retorno_col=ci
        date=None; prod=None; nf=None; client=None
        for i in range(hr+1,len(df)):
            row=df.iloc[i]
            # Forward-fill data
            if pd.notna(row.iloc[1]) and hasattr(row.iloc[1],"year"): date=row.iloc[1]
            # Cliente (col detectada) â€” forward-fill quando ausente
            if client_col<len(row) and pd.notna(row.iloc[client_col]):
                cv=str(row.iloc[client_col]).strip()
                if cv and cv.upper() not in ["","NAN","CLIENTE"]:
                    client=normalize_client(cv)
            # Produto
            p3=str(row.iloc[prod_col]).strip() if prod_col<len(row) and pd.notna(row.iloc[prod_col]) else ""
            if p3 and p3.upper() not in ["","NAN","PRODUTO","PRODUTOS"]: prod=p3
            # NF
            if nf_col<len(row) and pd.notna(row.iloc[nf_col]):
                nf_raw=str(row.iloc[nf_col]).strip()
                if nf_raw.lower() not in ["","nan","nf-e","nfe","nota"]: nf=nf_raw
            # Total / qt / preÃ§o
            if total_col>=len(row): continue
            tot=row.iloc[total_col]
            qt=row.iloc[total_col-2] if total_col>=2 else None
            pu=row.iloc[total_col-1] if total_col>=1 else None
            # Entregador + status (RETORNO NF E ENTREGADOR)
            driver=None; status_note=None
            if retorno_col is not None and retorno_col<len(row) and pd.notna(row.iloc[retorno_col]):
                driver, status_note = parse_retorno(row.iloc[retorno_col])
            if date and prod and pd.notna(tot):
                try:
                    t=float(tot); q=float(qt) if pd.notna(qt) else 0; p=float(pu) if pd.notna(pu) else 0
                    if t>0 and t<500000 and prod.upper() not in ["NAN","TOTAL DO MÃŠS","TOTAL DO MES"]:
                        recs.append(dict(sale_type="PR",sale_date=date.strftime("%Y-%m-%d"),
                            client=client,product=norm_p(prod,"PR"),nf_number=nf,
                            quantity=q,unit_price=p,total=t,
                            delivery_person=driver,notes=status_note,
                            source="excel"))
                except Exception: pass
    except Exception: pass
    return recs
...
def extract_avulsos(xl,sheet,prices=None):
    """
    Extrai avulsos detectando dinamicamente o layout de cada bloco e fazendo
    forward-fill de DATA e CLIENTE para capturar linhas de continuaÃ§Ã£o (mesmo
    cliente compra mÃºltiplos produtos no mesmo dia, sem repetir nome/data).
    Cada bloco encerra ao encontrar marcadores: TOTAL, PRETO, NA HORA, 1 SEMANA, PIX, 15D.
    """
    recs=[]
    try:
        df=pd.read_excel(xl,sheet_name=sheet,header=None)
        ...
        for i,row in df.iterrows():
            txt=" ".join(str(v).upper() for v in row.values if pd.notna(v))
            for mn,mv in MONTH_NAMES.items():
                if mn in txt: mes=mv; break
            if mes: break
        if mes is None:
            for i,row in df.iterrows():
                for v in row.values:
                    if pd.notna(v) and hasattr(v,"year") and v.year>=2024: mes=v.month; break
                if mes: break
        if mes is None: return []

        # Linha de clientes (cabeÃ§alhos de bloco)
        client_idx=None
        for i,row in df.iterrows():
            non_null=[str(v).strip() for v in row.values if pd.notna(v) and str(v).strip() not in ["","nan"]]
            has_date=any(hasattr(v,"year") for v in row.values if pd.notna(v))
            joined=" ".join(non_null).upper()
            if len(non_null)>=3 and not has_date and any(k in joined for k in CLIENT_KEYWORDS):
                client_idx=i; break
        if client_idx is None: return []
        crow=df.iloc[client_idx]

        # Detecta layout detalhado (DATA/PRODUTOS/VALOR) vs compacto (DATA+VALOR)
        detailed=False; header_idx=client_idx+1
        if header_idx<len(df):
            hlabels=[str(v).upper().strip() for v in df.iloc[header_idx].values if pd.notna(v)]
            if hlabels.count('DATA')>=1 and any('VALOR' in l for l in hlabels):
                detailed=True

        blocks=[]
        if detailed:
            hrow=df.iloc[header_idx]
            data_cols=[ci for ci,v in enumerate(hrow.values) if pd.notna(v) and str(v).upper().strip()=='DATA']
            for bi,dc in enumerate(data_cols):
                next_dc=data_cols[bi+1] if bi+1<len(data_cols) else dc+8
                cols={}
                for ci in range(dc,min(next_dc,len(hrow.values))):
                    v=hrow.iloc[ci]
                    if pd.notna(v):
                        lab=str(v).upper().strip()
                        if lab=='DATA':cols['date']=ci
                        elif lab=='CLIENTE':cols['cliente']=ci
                        elif 'PRODUTO' in lab:cols['produto']=ci
                        elif lab in('QT','QT.','QTD','QUANTIDADE'):cols['qt']=ci
                        elif 'UNIT' in lab:cols['punit']=ci
                        elif 'VALOR' in lab:cols['valor']=ci
                hc=None
                for ci in range(dc,min(next_dc,len(crow.values))):
                    v=crow.iloc[ci]
                    if pd.notna(v) and str(v).strip() not in('','nan'):hc=str(v).strip();break
                blocks.append({'cols':cols,'hc':hc,'last_date':None,'last_client':None,'ended':False,'range':(dc,next_dc)})
            data_start=header_idx+1
        else:
            # Compacto: cliente em col â†’ date=col, valor=col+1
            for ci,v in enumerate(crow.values):
                if pd.notna(v) and str(v).strip() not in('','nan'):
                    blocks.append({'cols':{'date':ci,'valor':ci+1},'hc':str(v).strip(),
                                   'last_date':None,'last_client':None,'ended':False,'range':(ci,ci+2)})
            data_start=client_idx+1

        END_MARKERS=("TOTAL","PRETO","NA HORA","1 SEMANA","PIX","15D")
        for i in range(data_start,len(df)):
            row=df.iloc[i]
            for bs in blocks:
                if bs['ended']: continue
                cols,hc=bs['cols'],bs['hc']
                if 'date' not in cols or 'valor' not in cols: continue
                r0,r1=bs['range']
                # Detecta marcador de fim no range do bloco
                block_cells=[str(row.iloc[c]).upper().strip() for c in range(r0,min(r1,len(row))) if pd.notna(row.iloc[c])]
                block_str=" ".join(block_cells)
                if any(m in block_str for m in END_MARKERS):
                    bs['ended']=True; continue
                # Forward-fill data (sÃ³ aceita do mÃªs detectado)
                if cols['date']<len(row):
                    dv=row.iloc[cols['date']]
                    if pd.notna(dv) and hasattr(dv,'year') and dv.year>=2024:
                        if dv.month==mes: bs['last_date']=dv
                        else: continue  # data de outro mÃªs â€” ignora
                # Forward-fill cliente
                if 'cliente' in cols and cols['cliente']<len(row):
                    cv=row.iloc[cols['cliente']]
                    if pd.notna(cv) and str(cv).strip() not in('','nan'):
                        bs['last_client']=str(cv).strip()
                # Valor
                if cols['valor']>=len(row): continue
                vv=row.iloc[cols['valor']]
                total=0.0; notes=None
                if pd.notna(vv):
                    try: total=float(vv)
                    except Exception:
                        notes=str(vv).strip()
                        m=re.findall(r"[\d]+[.,][\d]{2}",str(vv))
                        if m: total=float(m[0].replace(",","."))
                if total<=0 or total>=500000: continue
                if not bs['last_date']: continue  # sem contexto de data ainda
                # Cliente final: prioriza cliente do bloco, depois cabeÃ§alho do bloco
                client=bs['last_client'] or hc or 'AVULSO'
                client=normalize_client(client.strip())
                # Produto
                product="AVULSO"
                if 'produto' in cols and cols['produto']<len(row):
                    pv=row.iloc[cols['produto']]
                    if pd.notna(pv) and str(pv).strip() not in('','nan'): product=str(pv).strip()
                # Qt / preÃ§o unitÃ¡rio
                qt=0; punit=0
                if 'qt' in cols and cols['qt']<len(row):
                    try: qt=float(row.iloc[cols['qt']])
                    except Exception: pass
                if 'punit' in cols and cols['punit']<len(row):
                    try: punit=float(row.iloc[cols['punit']])
                    except Exception: pass
                recs.append(dict(sale_type="AVULSO",sale_date=bs['last_date'].strftime("%Y-%m-%d"),
                    client=client,product=product,nf_number=None,
                    quantity=qt,unit_price=punit,total=round(total,2),notes=notes,source="excel"))
    except Exception:
        pass
    return recs


def extract_avarias_from_sheet(xl,sheet,prices):
    recs=[]
    try:
        df=pd.read_excel(xl,sheet_name=sheet,header=None)
        hr=next((i for i,row in df.iterrows() if "DATA" in str(row.values).upper()),None)
        if hr is None: return []
        headers=[str(v).upper().strip() if pd.notna(v) else "" for v in df.iloc[hr].values]
        av_cols=[(ci,h) for ci,h in enumerate(headers) if "AVARIA" in h]
        if not av_cols: return []
        def cidx(kws):
            for kw in kws:
                for ci,h in enumerate(headers):
                    if kw in h: return ci
            return None
        col_pu=cidx(["P.UNIT","PRECO","PREÃ‡O","UNIT"])
        date=client=prod=None; pu=0.0
        for i in range(hr+1,len(df)):
            row=df.iloc[i]
            if pd.notna(row.iloc[1]) and hasattr(row.iloc[1],"year"): date=row.iloc[1]
            if pd.notna(row.iloc[2]) and str(row.iloc[2]).strip() not in ["","nan"]: client=str(row.iloc[2]).strip()
            if pd.notna(row.iloc[3]) and str(row.iloc[3]).strip() not in ["","nan","PRODUTO"]: prod=str(row.iloc[3]).strip()
            if col_pu and pd.notna(row.iloc[col_pu]):
                try: pu=float(row.iloc[col_pu])
                except Exception: pass
            for av_ci,av_h in av_cols:
                if av_ci>=len(row): continue
                cell=row.iloc[av_ci]
                if not pd.notna(cell) or str(cell).strip() in ["","nan","0"]: continue
                cell_str=str(cell).strip()
                try:
                    q=float(cell_str)
                    if q>0 and pu>0 and date:
                        recs.append(dict(sale_type="AVARIA",sale_date=date.strftime("%Y-%m-%d"),
                            client=client,product=norm_p(prod or ""),nf_number=None,
                            quantity=q,unit_price=pu,total=round(q*pu,2),notes=f"QT:{q} x R${pu:.2f}",source="excel"))
                except Exception:
                    total_v,items=parse_avaria_text(cell_str,prices)
                    for it in items:
                        if date:
                            recs.append(dict(sale_type="AVARIA",sale_date=date.strftime("%Y-%m-%d"),
                                client=client,product=it["product"],nf_number=None,
                                quantity=it["qty"],unit_price=it["price"],total=it["value"],
                                notes=f"Avaria: {cell_str}",source="excel"))
    except Exception: pass
    return recs

@app.post("/api/import-excel")
async def import_excel(file:UploadFile=File(...),x_token:str=Header("")):
    sess=require_editor_tab_access(x_token,["cfg_importar","consolidado"])
    fname=(file.filename or "").lower()
    if not fname.endswith((".xls",".xlsx")):
        raise HTTPException(400,"Envie uma planilha .xls ou .xlsx.")
    content=await read_upload_limited(file, MAX_EXCEL_UPLOAD, "Planilha")
    engine="xlrd" if fname.endswith(".xls") else "openpyxl"
    xl=pd.ExcelFile(io.BytesIO(content),engine=engine)
    prices=load_prices(); all_recs=[]
    skip_kw=["CONSOLIDADO","MACAXEIRA &","PLANEJAMENTO","ðŸ“Š","ðŸŒ¿","ðŸ“…","âš ","VALORES_PRODUTOS"]
    for sheet in xl.sheet_names:
        su=sheet.upper()
        if any(k in su for k in skip_kw): continue
        if re.search(r"QT\.?\s*AVARIA",su): all_recs+=extract_avarias_from_sheet(xl,sheet,prices)
        elif su.startswith("NF"):
            all_recs+=extract_nf(xl,sheet)
            all_recs+=extract_avarias_from_sheet(xl,sheet,prices)
        elif su.startswith("PR") and "AVULSO" not in su: all_recs+=extract_pr(xl,sheet)
